<?php
include '../koneksi.php';

// Fetching data from transaksi table
$query = "
  SELECT transaksi.id AS id, produk.bar AS bar, produk.nama AS nama, transaksi.harga AS harga, toko.nama AS toko 
  FROM transaksi 
  JOIN produk ON transaksi.prod_id = produk.bar 
  JOIN toko ON transaksi.toko_id = toko.id
";
$result = $kon->query($query);
$datas = [];

// Check if any rows were returned
if ($result->num_rows > 0) {
  while ($row = $result->fetch_assoc()) {
    $datas[] = $row;
  }
  $hasil = json_encode($datas);
  echo "<script>console.log('Data: $hasil')</script>";
} else {
  echo "<script>console.log('No data found')</script>";
}

// Handle deletion request
if (isset($_GET['delete'])) {
  $id = $_GET['delete'];

  // Prepare and execute the delete statement
  if ($stmt = $kon->prepare("DELETE FROM transaksi WHERE id = ?")) {
    $stmt->bind_param("i", $id); // $id is an integer
    $stmt->execute();
    $stmt->close();
    echo "<script>alert('Data deleted successfully'); window.location.href='index.php';</script>";
  }
}

$kon->close();
?>

<!DOCTYPE html>
<html lang="en">

<head>
  <?php include '../layout/head.php'; ?>
  <title>Data Transaksi</title>
</head>

<body class="g-sidenav-show bg-gray-100">
  <div class="min-height-600 bg-success position-absolute w-100"></div>
  <?php
  $active = "transaksi";
  include '../layout/sidebar.php';
  ?>
  <main class="main-content position-relative border-radius-lg vh-100">
    <!-- Navbar -->
    <?php
    $judul = "Data Transaksi";
    include '../layout/navbar.php';
    ?>
    <!-- End Navbar -->
    <div class="container-fluid py-4">
      <div class="row">
        <div class="col-12">
          <div class="card mb-4">
            <div class="card-head text-end">
              <a href="<?= $url ?>transaksi/tambah.php" class="btn btn-icon btn-3 btn-success m-2" type="button">
                <span class="btn-inner--icon"><i class="fa-solid fa-plus"></i></span>
                <span class="btn-inner--text">Tambahkan</span>
              </a>
            </div>
            <div class="card-body px-0 pt-0 pb-2">
              <div class="table-responsive p-0">
                <table class="table align-items-center mb-0">
                  <thead>
                    <tr>
                      <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-3">Produk</th>
                      <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Harga</th>
                      <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Toko</th>
                      <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Aksi</th>
                    </tr>
                  </thead>
                  <tbody>
                    <?php foreach ($datas as $data): ?>
                      <tr>
                        <td>
                          <div class="d-flex px-2 py-1">
                            <div class="d-flex flex-column justify-content-center">
                              <p class="mb-0 text-xs text-wrap"><?= $data['nama']; ?></p>
                            </div>
                          </div>
                        </td>
                        <td>
                          <p class="text-xs text-secondary mb-0 text-wrap"><?= $data['harga']; ?></p>
                        </td>
                        <td>
                          <p class="text-xs text-secondary mb-0 text-wrap"><?= $data['toko']; ?></p>
                        </td>
                        <td class="align-middle text-center">
                          <a href="<?= $url ?>/transaksi/edit.php?id=<?= urlencode($data['id']); ?>" class="edit btn btn-info m-0">Edit</a>
                          <button class="hapus btn btn-danger m-0" data-bs-toggle="modal" data-bs-target="#modal-default" data-id="<?= $data['id']; ?>">Hapus</button>
                        </td>
                      </tr>
                    <?php endforeach; ?>
                  </tbody>
                </table>
                <!-- Modal for Deletion Confirmation -->
                <div class="modal fade" id="modal-default" tabindex="-1" role="dialog" aria-labelledby="modal-default" aria-hidden="true">
                  <div class="modal-dialog modal- modal-dialog-centered modal-" role="document">
                    <div class="modal-content">
                      <div class="modal-header">
                        <h6 class="modal-title" id="modal-title-default">Yakin Ingin Menghapus</h6>
                      </div>
                      <div class="modal-footer">
                        <a href="#" id="delete-link" class="btn bg-gradient-danger">Ya Hapus</a>
                        <button type="button" class="btn btn-link ml-auto" data-bs-dismiss="modal">Kembali</button>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <footer class="footer px-3 position-absolute bottom-2">
          <div class="container-fluid">
            <div class="row align-items-center justify-content-lg-between">
            </div>
          </div>
        </footer>
      </div>
  </main>
  <?php include '../layout/scripts.php' ?>
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      // Adjust layout for smaller screens
      if (window.innerWidth <= 425) {
        const phones = document.getElementsByClassName('phone');
        for (let phone of phones) {
          phone.hidden = true;
        }

        const edits = document.getElementsByClassName('edit');
        for (let edit of edits) {
          edit.className = "edit btn btn-info m-0 p-1";
          edit.innerHTML = "<i class='ni ni-ruler-pencil'></i>";
        }

        const hapus = document.getElementsByClassName('hapus');
        for (let button of hapus) {
          button.className = "hapus btn btn-danger m-0 p-1";
          button.innerHTML = "<i class='ni ni-fat-remove'></i>";
        }
      }

      // Modal event listener for deletion
      const modal = document.getElementById('modal-default');
      modal.addEventListener('show.bs.modal', function(event) {
        const button = event.relatedTarget;
        const id = button.getAttribute('data-id');
        const deleteLink = document.getElementById('delete-link');
        deleteLink.href = `<?= $url ?>/transaksi/?delete=${encodeURIComponent(id)}`;
      });
    });
  </script>
</body>

</html>